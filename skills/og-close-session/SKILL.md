...
description: |
  ...
  Companion: /og-orient opens a session; /og-close-session ends it.
---
...
File into the org's **ratified communication substrate** — for an org on the `memos/` default, at `memos/<YYYY-MM-DD>-<HHMM>--<seat>--<seat>--<short-summary>.{openthing,body.md}`; on another substrate, whatever that substrate's equivalent is. **Discharge as you go:** if this session completed, declined, superseded or overtook an action another seat asked for, mark the original artifact additively (`metadata.discharged` with `on`, `by`, `via`, `disposition`) rather than leaving it flagged forever. Never flip `action_required` — it was true when filed and stays true of the artifact as filed. `from` and `to` both name the seat you occupied (institutional capture for that seat's history — your replacements read it on their next `/og-orient`). If unattached (no seat), use `unattached-ai` and note it in `metadata.scope_check`.
...
## References
- Companion: [/og-orient](../og-orient/SKILL.md) (opens the session) · [/og-describe-org](../og-describe-org/SKILL.md)
