...
description: |
  ...
  come-up-to-speed read. /og-describe-org is the standalone, anytime view.
---
...
- The org's **ratified communication substrate**, newest-first — the last ~10, with dates. The charter (`governance_model`) or `CLAUDE.md` names it; `memos/` is OG's default, not a requirement. **Report what is OPEN**, not everything ever flagged: an open item is `action_required: true` with **no** `metadata.discharged`. A list of every item ever actioned is an archive, not an inbox.
...
## References
- Companion: [/og-orient](../og-orient/SKILL.md) (emits this snapshot after a deeper read) · [/og-close-session](../og-close-session/SKILL.md)
