...
- **Seat-inbox-notification ("doorbell")** (2026-06-15) — transport-neutral convention: a memo filed to a *warm* seat SHOULD notify/trigger it (ends the human "please check for memos" handoff). Claude Code channels recorded as one reference transport (Path A validated live; four gates fix it as non-canonical); vendor-neutral transports (file-watch/poll/MCP-inbox) carry none of the gates. Decision: [proposal-seat-inbox-notification-doorbell.md](decisions/proposal-seat-inbox-notification-doorbell.md). No charter bump (promotion held under #8).

### Active (strategist)

- **Doorbell promotion watch** (revisit, don't re-decide) — seat-inbox-notification convention decided + channels transport validated (Path A); promotion to `recommended_patterns.general` earned only after Path B PoC (channels) **and** a vendor-neutral transport (file-watch/poll) both land, proving transport-independence (conformance #3).
...
- **Path B doorbell PoC** (memos/2026-06-15-1150) — Node `memos/`-watcher custom channel that rings a running seat on new-memo arrival (no Bun, no HTTP; runs via `--dangerously-load-development-channels server:memo-watch`); pointer-not-payload + vendor-neutral-structuring invariants set by strategist; awaits a staffed implementer session + Director trigger
